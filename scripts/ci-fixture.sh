...
export FIXTURE_SHA="$sha" FIXTURE_OUT="$out"
collect() {
  result=$?
  trap - EXIT
  set +e
  ...
  adb shell dumpsys accessibility > "$out/diagnostics/accessibility.txt" 2>&1
  adb shell dumpsys window windows > "$out/diagnostics/windows.txt" 2>&1
  adb shell dumpsys activity activities > "$out/diagnostics/activities.txt" 2>&1
  adb logcat -d -s AndroidRuntime:E > "$out/diagnostics/crashes.txt" 2>&1
...
for label, pattern in [("unit", "**/test-results/testDebugUnitTest/TEST-*.xml"), ("device", "**/outputs/androidTest-results/connected/**/TEST-*.xml")]:
    ...
    if any(int(s.get("failures", 0)) or int(s.get("errors", 0)) or int(s.get("skipped", 0)) for s in suites):
