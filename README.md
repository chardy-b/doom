...
- **Diagnostic only:** counts and tree shape vary with scrolling and content. A deterministic shadow classifier may emit a surface prediction from strong, unambiguous structural tokens, but it is non-blocking and has no confidence, live gate, protection, or navigation effect. Messaging takes precedence; truncated, mixed, unknown, and ambiguous reports fail open to `UNKNOWN`. Bounded single-device observations exist on Pixel 11 Pro / Android 17 / Instagram 445.0.0.45.83; cross-version stability and the repeated safety/latency matrix remain unverified.
- **Observe Instagram only:** a consenting, connected service visits at most 128 nodes, breadth-first, at depths 0 through 8. No Instagram launch, navigation, node actions, live gate or overlay exists in `:app`. Non-Instagram events return before reading an active root. A delayed Instagram event with a wrong/null root or a collection failure invalidates the report. Foreign or unattributed child nodes consume the node budget but are skipped before reading report metadata or children, marking truncation. Nodes are transient and recycled, including failure paths.
...
- **Memory and consent:** only `sanitized_structural_report_v1` consent persists. Old consent cannot authorize this diagnostic. The current report, reveal state and copy-success state start empty in a new process. Clear, revoke, observer stop, disconnect, interruption, reconnect and process death discard all three. Interruption disables recording until a service connection clears state and rechecks consent. Clear leaves consent enabled; a later Instagram event may create a new hidden report. Returning to Doom normally preserves the latest valid report for review. Each new report, including identical content, requires a new reveal before copying.
