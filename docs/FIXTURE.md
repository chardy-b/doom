...
Six standalone JVM tests exercise the actual `GatePolicy`: exact deadline/session credit, each non-Feed cancellation, stale replacement completion, navigation suppression, cancellation reset, and repeated-event deadline stability. Six device tests exercise actual cross-app windows and UI: DM escape, completion/new session, Unknown/other app, actual service disable, background/lock/Leave, and consent revocation/rejection. The earlier supplied run failed in device-test cleanup; the latest supplied run failed in preparation before fixture compilation or tests. Both repairs still require a new-SHA CI run and evidence review.
