...
| Debug exported test/preview activities | Resolved: removed `ui-test-manifest` and debug-only `ui-tooling`, which supplied exported debug activities not required by the current instrumentation tests. `ui-tooling-preview` remains for `@Preview` annotations. CI instrumentation queries the installed package with `PackageManager` and verifies that the merged APK contains neither `androidx.compose.ui.tooling.PreviewActivity` nor `androidx.activity.ComponentActivity`, and that `MainActivity` is the only exported activity. Dependency receivers outside this preview/test-activity scope remain permission-protected. |
...
The host guard is `scripts/test_wil155_host.py`; merged activity removal is verified by the Android instrumentation test in CI. Android/Gradle/device execution remains CI-only by project policy.
