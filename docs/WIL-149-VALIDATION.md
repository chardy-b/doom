...
- The ten supplementary names are CI-bound scenarios: the first five exercise Doom-owned overlay states, the sixth is the scrollable MainActivity build-footer screen, and names seven through ten show the expanded timer and dismiss control, compact app-drawn icon after synthetic drag/settle, and the actual dashboard timer switch disabled then re-enabled. They remain synthetic UI evidence and are not claimed as executed until exact-head CI uploads the manifest.
