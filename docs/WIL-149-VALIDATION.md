...
The current report contract is version 2 and requires fresh consent; the removed v1 report consent never authorizes this collection. With both consents and a connected observer, Doom keeps one process-memory, bounded, indexed structural snapshot of an Instagram root: at most 128 nodes breadth-first through depth 8, parent/depth/BFS/sibling indexes, sanitized resource/class identifiers, screen and API-34+ window bounds, normalized screen bounds, drawing order, fixed action names/IDs, collection/item/range tuples, named numeric fields, and closed boolean masks. Unsupported, absent, invalid, free-form, or capped values are represented by closed `u:*` markers. Android framework objects, text, descriptions, labels, extras, arbitrary identifiers and private screenshots are never retained; metadata is evidence only and never selection or action authority. The public unique ID is copied only when it exactly equals an already accepted safe resource ID.

The shared reminder bloom uses a 32-pitch lattice, continuous float rectangles, a 20%→87.5% width envelope, smootherstep organic stagger, one ivory center, and fixed 4-second inhale/6-second exhale timing. Reduced motion fixes geometry while phase/progress remain live. The reminder has no report or status data. Its small secondary **Debug report** action remains below dominant Skip and Leave; it requires current authority and confirmed physical detachment, preserves only the current hidden report, opens Doom's explicit Debug journey, and never earns terminal cooldown. Skip remains the sole Instagram action and still targets only the exact Direct-tab ID after detachment and all authority checks. No new permission, event capability, persistence, network, upload, or evidence lane is added.
...
- The ten supplementary names are CI-bound scenarios: the first five exercise Doom-owned overlay states, the sixth is the scrollable MainActivity build-footer screen, and names seven through ten show the expanded timer and dismiss control, compact app-drawn icon rendering, and the actual dashboard timer Switch clicked to disabled and then clicked back to enabled with checked-state assertions. Screenshot 08 does not exercise or claim production drag/snap behavior. They remain synthetic UI evidence and are not claimed as executed until exact-head CI uploads the manifest.

Exactly three booleans persist: the two existing explicit consents and the default-enabled Instagram session-timer preference. Only the timer preference is new. No elapsed time, history, position, visit dismissal, removal veto, event data, or Instagram data is persisted.
